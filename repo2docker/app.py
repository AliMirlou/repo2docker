        ...
        try:
            ...
            with chdir(checkout_path):
                ...
                if self.dry_run:
                    ...
                    self.log.info(
                        f"Using {picked_buildpack.__class__.__name__} builder\n",
                        extra=dict(phase=R2dState.BUILDING),
                    )
                    ...
                    for build_result in picked_buildpack.build(
                        ...
                        if docker_client.string_output:
                            self.log.info(build_result, extra=dict(phase=R2dState.BUILDING))
                        # else this is Docker output
                        elif "stream" in build_result:
                            self.log.info(
                                build_result["stream"], extra=dict(phase=R2dState.BUILDING)
                            )
                        elif "error" in build_result:
                            self.log.info(build_result["error"], extra=dict(phase=R2dState.FAILED))
                            ...
                            self.log.info(
                                json.dumps(build_result), extra=dict(phase=R2dState.BUILDING)
                            )
